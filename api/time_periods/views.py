"""
TimePeriods views matching FastAPI behavior exactly.
Implements all time_periods endpoints from FastAPI time_periods_api.py
"""
...
from .serializers import (
    TimePeriodCompactSerializer,
    TimePeriodResponseSerializer,
)


class TimePeriodsViewSet(viewsets.ViewSet):
    """
    TimePeriods viewset matching FastAPI time_periods_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /time_periods
        Returns the compact records for all time_periods.
        """
        ...
        if page is not None:
            serializer = TimePeriodCompactSerializer(page, many=True)
        ...
        serializer = TimePeriodCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /time_periods/{id}
        Returns the complete record for a single time_periods resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('TimePeriod')
        ...
        try:
            resource = TimePeriod.objects.get(gid=resource_gid)
        except TimePeriod.DoesNotExist:
            return asana_not_found_error('TimePeriod')
        
        serializer = TimePeriodResponseSerializer(resource)
        data = serializer.data
