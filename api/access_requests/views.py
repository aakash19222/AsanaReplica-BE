"""
AccessRequests views matching FastAPI behavior exactly.
Implements all access_requests endpoints from FastAPI access_requests_api.py
"""
...
from .serializers import (
    AccessRequestCompactSerializer,
    AccessRequestResponseSerializer,
)


class AccessRequestsViewSet(viewsets.ViewSet):
    """
    AccessRequests viewset matching FastAPI access_requests_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /access_requests
        Returns the compact records for all access_requests.
        """
        ...
        if page is not None:
            serializer = AccessRequestCompactSerializer(page, many=True)
        ...
        serializer = AccessRequestCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        ...
        Returns the complete record for a single access_requests resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('AccessRequest')
        ...
        try:
            resource = AccessRequest.objects.get(gid=resource_gid)
        except AccessRequest.DoesNotExist:
            return asana_not_found_error('AccessRequest')
        
        serializer = AccessRequestResponseSerializer(resource)
        data = serializer.data
