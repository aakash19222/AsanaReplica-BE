"""
Goals views matching FastAPI behavior exactly.
Implements all goals endpoints from FastAPI goals_api.py
"""
...
class GoalsViewSet(viewsets.ViewSet):
    """
    Goals viewset matching FastAPI goals_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /goals
        Returns the compact records for all goals.
        """
        ...
        if page is not None:
            serializer = GoalCompactSerializer(page, many=True)
        ...
        serializer = GoalCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /goals/{id}
        Returns the complete record for a single goals resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Goal')
        ...
        try:
            resource = Goal.objects.get(gid=resource_gid)
        except Goal.DoesNotExist:
            return asana_not_found_error('Goal')
        
        serializer = GoalResponseSerializer(resource)
        data = serializer.data
