"""
TeamMemberships views matching FastAPI behavior exactly.
Implements all team_memberships endpoints from FastAPI team_memberships_api.py
"""
...
from .serializers import (
    TeamMembershipCompactSerializer,
    TeamMembershipResponseSerializer,
)


class TeamMembershipsViewSet(viewsets.ViewSet):
    """
    TeamMemberships viewset matching FastAPI team_memberships_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        ...
        Returns the compact records for all team_memberships.
        """
        ...
        if page is not None:
            serializer = TeamMembershipCompactSerializer(page, many=True)
        ...
        serializer = TeamMembershipCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        ...
        Returns the complete record for a single team_memberships resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('TeamMembership')
        ...
        try:
            resource = TeamMembership.objects.get(gid=resource_gid)
        except TeamMembership.DoesNotExist:
            return asana_not_found_error('TeamMembership')
        
        serializer = TeamMembershipResponseSerializer(resource)
        data = serializer.data
