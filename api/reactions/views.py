"""
Reactions views matching FastAPI behavior exactly.
Implements all reactions endpoints from FastAPI reactions_api.py
"""
...
from .serializers import (
    ReactionCompactSerializer,
    ReactionResponseSerializer,
)


class ReactionsViewSet(viewsets.ViewSet):
    """
    Reactions viewset matching FastAPI reactions_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /reactions
        Returns the compact records for all reactions.
        """
        ...
        if page is not None:
            serializer = ReactionCompactSerializer(page, many=True)
        ...
        serializer = ReactionCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /reactions/{id}
        Returns the complete record for a single reactions resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Reaction')
        ...
        try:
            resource = Reaction.objects.get(gid=resource_gid)
        except Reaction.DoesNotExist:
            return asana_not_found_error('Reaction')
        
        serializer = ReactionResponseSerializer(resource)
        data = serializer.data
