"""
ProjectTemplates views matching FastAPI behavior exactly.
Implements all project_templates endpoints from FastAPI project_templates_api.py
"""
...
from .serializers import (
    ProjectTemplateCompactSerializer,
    ProjectTemplateResponseSerializer,
)


class ProjectTemplatesViewSet(viewsets.ViewSet):
    """
    ProjectTemplates viewset matching FastAPI project_templates_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        ...
        Returns the compact records for all project_templates.
        """
        ...
        if page is not None:
            serializer = ProjectTemplateCompactSerializer(page, many=True)
        ...
        serializer = ProjectTemplateCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        ...
        Returns the complete record for a single project_templates resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('ProjectTemplate')
        ...
        try:
            resource = ProjectTemplate.objects.get(gid=resource_gid)
        except ProjectTemplate.DoesNotExist:
            return asana_not_found_error('ProjectTemplate')
        
        serializer = ProjectTemplateResponseSerializer(resource)
        data = serializer.data
