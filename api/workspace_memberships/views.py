"""
WorkspaceMemberships views matching FastAPI behavior exactly.
Implements all workspace_memberships endpoints from FastAPI workspace_memberships_api.py
"""
...
from .serializers import (
    WorkspaceMembershipCompactSerializer,
    WorkspaceMembershipResponseSerializer,
)


class WorkspaceMembershipsViewSet(viewsets.ViewSet):
    """
    WorkspaceMemberships viewset matching FastAPI workspace_memberships_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        ...
        Returns the compact records for all workspace_memberships.
        """
        ...
        if page is not None:
            serializer = WorkspaceMembershipCompactSerializer(page, many=True)
        ...
        serializer = WorkspaceMembershipCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /workspace_memberships/{id}
        Returns the complete record for a single workspace_memberships resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('WorkspaceMembership')
        ...
        try:
            resource = WorkspaceMembership.objects.get(gid=resource_gid)
        except WorkspaceMembership.DoesNotExist:
            return asana_not_found_error('WorkspaceMembership')
        
        serializer = WorkspaceMembershipResponseSerializer(resource)
        data = serializer.data
