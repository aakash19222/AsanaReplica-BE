"""
CustomTypes views matching FastAPI behavior exactly.
Implements all custom_types endpoints from FastAPI custom_types_api.py
"""
...
from .serializers import (
    CustomTypeCompactSerializer,
    CustomTypeResponseSerializer,
)


class CustomTypesViewSet(viewsets.ViewSet):
    """
    CustomTypes viewset matching FastAPI custom_types_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /custom_types
        Returns the compact records for all custom_types.
        """
        ...
        if page is not None:
            serializer = CustomTypeCompactSerializer(page, many=True)
        ...
        serializer = CustomTypeCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /custom_types/{id}
        Returns the complete record for a single custom_types resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('CustomType')
        ...
        try:
            resource = CustomType.objects.get(gid=resource_gid)
        except CustomType.DoesNotExist:
            return asana_not_found_error('CustomType')
        
        serializer = CustomTypeResponseSerializer(resource)
        data = serializer.data
