"""
Exports views matching FastAPI behavior exactly.
Implements all exports endpoints from FastAPI exports_api.py
"""
...
from .serializers import (
    ExportCompactSerializer,
    ExportResponseSerializer,
)


class ExportsViewSet(viewsets.ViewSet):
    """
    Exports viewset matching FastAPI exports_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /exports
        Returns the compact records for all exports.
        """
        ...
        if page is not None:
            serializer = ExportCompactSerializer(page, many=True)
        ...
        serializer = ExportCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /exports/{id}
        Returns the complete record for a single exports resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Export')
        ...
        try:
            resource = Export.objects.get(gid=resource_gid)
        except Export.DoesNotExist:
            return asana_not_found_error('Export')
        
        serializer = ExportResponseSerializer(resource)
        data = serializer.data
