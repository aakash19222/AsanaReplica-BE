"""
ProjectStatuses views matching FastAPI behavior exactly.
Implements all project_statuses endpoints from FastAPI project_statuses_api.py
"""
...
from .serializers import (
    ProjectStatusCompactSerializer,
    ProjectStatusResponseSerializer,
)


class ProjectStatusesViewSet(viewsets.ViewSet):
    """
    ProjectStatuses viewset matching FastAPI project_statuses_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        ...
        Returns the compact records for all project_statuses.
        """
        ...
        if page is not None:
            serializer = ProjectStatusCompactSerializer(page, many=True)
        ...
        serializer = ProjectStatuseCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        ...
        Returns the complete record for a single project_statuses resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('ProjectStatus')
        ...
        try:
            resource = ProjectStatus.objects.get(gid=resource_gid)
        except ProjectStatus.DoesNotExist:
            return asana_not_found_error('ProjectStatus')
        
        serializer = ProjectStatusResponseSerializer(resource)
        data = serializer.data
