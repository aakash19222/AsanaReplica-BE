"""
Memberships views matching FastAPI behavior exactly.
Implements all memberships endpoints from FastAPI memberships_api.py
"""
...
class MembershipsViewSet(viewsets.ViewSet):
    """
    Memberships viewset matching FastAPI memberships_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /memberships
        Returns the compact records for all memberships.
        """
        ...
        if page is not None:
            serializer = MembershipCompactSerializer(page, many=True)
        ...
        serializer = MembershipCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /memberships/{id}
        Returns the complete record for a single memberships resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Membership')
        ...
        try:
            resource = Membership.objects.get(gid=resource_gid)
        except Membership.DoesNotExist:
            return asana_not_found_error('Membership')
        
        serializer = MembershipResponseSerializer(resource)
        data = serializer.data
