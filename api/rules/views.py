"""
Rules views matching FastAPI behavior exactly.
Implements all rules endpoints from FastAPI rules_api.py
"""
...
from .serializers import (
    RuleCompactSerializer,
    RuleResponseSerializer,
)


class RulesViewSet(viewsets.ViewSet):
    """
    Rules viewset matching FastAPI rules_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /rules
        Returns the compact records for all rules.
        """
        ...
        if page is not None:
            serializer = RuleCompactSerializer(page, many=True)
        ...
        serializer = RuleCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /rules/{id}
        Returns the complete record for a single rules resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Rule')
        ...
        try:
            resource = Rule.objects.get(gid=resource_gid)
        except Rule.DoesNotExist:
            return asana_not_found_error('Rule')
        
        serializer = RuleResponseSerializer(resource)
        data = serializer.data
