"""
Events views matching FastAPI behavior exactly.
Implements all events endpoints from FastAPI events_api.py
"""
...
class EventsViewSet(viewsets.ViewSet):
    """
    Events viewset matching FastAPI events_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /events
        Returns the compact records for all events.
        """
        ...
        if page is not None:
            serializer = EventCompactSerializer(page, many=True)
        ...
        serializer = EventCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /events/{id}
        Returns the complete record for a single events resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Event')
        ...
        try:
            resource = Event.objects.get(gid=resource_gid)
        except Event.DoesNotExist:
            return asana_not_found_error('Event')
        
        serializer = EventResponseSerializer(resource)
        data = serializer.data
