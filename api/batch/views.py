"""
Batch views matching FastAPI behavior exactly.
Implements all batch endpoints from FastAPI batch_api.py
"""
...
class BatchViewSet(viewsets.ViewSet):
    """
    Batch viewset matching FastAPI batch_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /batch
        Returns the compact records for all batch.
        """
        ...
        # Query from database
        queryset = BatchRequest.objects.all()
        ...
        if page is not None:
            serializer = BatchCompactSerializer(page, many=True)
        ...
        serializer = BatchCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /batch/{id}
        Returns the complete record for a single batch resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Batch')
        ...
        try:
            resource = BatchRequest.objects.get(gid=resource_gid)
        except BatchRequest.DoesNotExist:
            return asana_not_found_error('Batch')
        
        serializer = BatchResponseSerializer(resource)
        data = serializer.data
