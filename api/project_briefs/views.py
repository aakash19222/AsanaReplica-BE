"""
ProjectBriefs views matching FastAPI behavior exactly.
Implements all project_briefs endpoints from FastAPI project_briefs_api.py
"""
...
from .serializers import (
    ProjectBriefCompactSerializer,
    ProjectBriefResponseSerializer,
)


class ProjectBriefsViewSet(viewsets.ViewSet):
    """
    ProjectBriefs viewset matching FastAPI project_briefs_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /project_briefs
        Returns the compact records for all project_briefs.
        """
        ...
        if page is not None:
            serializer = ProjectBriefCompactSerializer(page, many=True)
        ...
        serializer = ProjectBriefCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /project_briefs/{id}
        Returns the complete record for a single project_briefs resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('ProjectBrief')
        ...
        try:
            resource = ProjectBrief.objects.get(gid=resource_gid)
        except ProjectBrief.DoesNotExist:
            return asana_not_found_error('ProjectBrief')
        
        serializer = ProjectBriefResponseSerializer(resource)
        data = serializer.data
