"""
Allocations views matching FastAPI behavior exactly.
Implements all allocations endpoints from FastAPI allocations_api.py
"""
...
from .serializers import (
    AllocationCompactSerializer,
    AllocationResponseSerializer,
)


class AllocationsViewSet(viewsets.ViewSet):
    """
    Allocations viewset matching FastAPI allocations_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /allocations
        Returns the compact records for all allocations.
        """
        ...
        if page is not None:
            serializer = AllocationCompactSerializer(page, many=True)
        ...
        serializer = AllocationCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /allocations/{id}
        Returns the complete record for a single allocations resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Allocation')
        ...
        try:
            resource = Allocation.objects.get(gid=resource_gid)
        except Allocation.DoesNotExist:
            return asana_not_found_error('Allocation')
        
        serializer = AllocationResponseSerializer(resource)
        data = serializer.data
