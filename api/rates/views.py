"""
Rates views matching FastAPI behavior exactly.
Implements all rates endpoints from FastAPI rates_api.py
"""
...
class RatesViewSet(viewsets.ViewSet):
    """
    Rates viewset matching FastAPI rates_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /rates
        Returns the compact records for all rates.
        """
        ...
        if page is not None:
            serializer = RateCompactSerializer(page, many=True)
        ...
        serializer = RateCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /rates/{id}
        Returns the complete record for a single rates resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('Rate')
        ...
        try:
            resource = Rate.objects.get(gid=resource_gid)
        except Rate.DoesNotExist:
            return asana_not_found_error('Rate')
        
        serializer = RateResponseSerializer(resource)
        data = serializer.data
