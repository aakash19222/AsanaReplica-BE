"""
UserTaskLists views matching FastAPI behavior exactly.
Implements all user_task_lists endpoints from FastAPI user_task_lists_api.py
"""
...
from .serializers import (
    UserTaskListCompactSerializer,
    UserTaskListResponseSerializer,
)


class UserTaskListsViewSet(viewsets.ViewSet):
    """
    UserTaskLists viewset matching FastAPI user_task_lists_api.py behavior.
    """
    required_scopes = []
    ...
    def list(self, request: Request) -> Response:
        """
        GET /user_task_lists
        Returns the compact records for all user_task_lists.
        """
        ...
        # Query from database
        queryset = UserTaskList.objects.all()
        ...
        if page is not None:
            serializer = UserTaskListCompactSerializer(page, many=True)
        ...
        serializer = UserTaskListCompactSerializer(queryset, many=True)
        data = serializer.data
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        """
        GET /user_task_lists/{id}
        Returns the complete record for a single user_task_lists resource.
        """
        resource_gid = pk
        if not resource_gid:
            return asana_not_found_error('UserTaskList')
        ...
        try:
            resource = UserTaskList.objects.get(gid=resource_gid)
        except UserTaskList.DoesNotExist:
            return asana_not_found_error('UserTaskList')
        
        serializer = UserTaskListResponseSerializer(resource)
        data = serializer.data
